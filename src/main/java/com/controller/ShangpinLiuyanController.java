...
public class ShangpinLiuyanController {
    ...
    */
    ...
    public R update(@RequestBody ShangpinLiuyanEntity shangpinLiuyan, HttpServletRequest request){
...
//            shangpinLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<ShangpinLiuyanEntity> queryWrapper = new EntityWrapper<ShangpinLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        ShangpinLiuyanEntity shangpinLiuyanEntity = shangpinLiuyanService.selectOne(queryWrapper);
        shangpinLiuyan.setUpdateTime(new Date());
        if(shangpinLiuyanEntity==null){
...
//                            shangpinLiuyanEntity.setShangpinId(Integer.valueOf(data.get(0)));   //商品 要改的

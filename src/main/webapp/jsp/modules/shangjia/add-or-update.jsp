                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 shangjiaNameDiv">
                                        <label>商家名称</label>
                                        <input style="width: 450px" id="shangjiaName" name="shangjiaName" class="form-control"
                                               placeholder="商家名称">
                                    </div>
                                    <div class="form-group col-md-6 shangjiaPhoneDiv">
                                        <label>联系方式</label>
                                        <input style="width: 450px" id="shangjiaPhone" name="shangjiaPhone" class="form-control"
                                               onchange="shangjiaPhoneChickValue(this)"  placeholder="联系方式">
                                    </div>
                                    <div class="form-group col-md-6 shangjiaEmailDiv">
                                        <label>邮箱</label>
                                        <input style="width: 450px" id="shangjiaEmail" name="shangjiaEmail" class="form-control"
                                               placeholder="邮箱">
                                    </div>
                                    <div class="form-group col-md-6 shangjiaPhotoDiv">
                                        <label>营业执照展示</label>
                                        <img id="shangjiaPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="shangjiaPhotoupload"
                                               class="form-control-file">
                                        <input name="shangjiaPhoto" id="shangjiaPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6 shangjiaXingjiTypesDiv">
                                        <label>商家信用类型</label>
                                        <select style="width: 450px" id="shangjiaXingjiTypesSelect" name="shangjiaXingjiTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6 newMoneyDiv">
                                        <label>现有余额</label>
                                        <input style="width: 450px" id="newMoney" name="newMoney" class="form-control"
                                               onchange="newMoneyChickValue(this)" placeholder="现有余额">
                                    </div>
                                    <div class="form-group  col-md-6 shangjiaContentDiv">
                                        <label>商家简介</label>
...
<script>
    ...
    function upload() {
        ...
        $('#shangjiaPhotoupload').fileupload({
            ...
            dataType: 'json',
                ...
                document.getElementById('shangjiaPhotoImg').setAttribute('src',photoUrl);
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("shangjia/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addshangjia');
    ...
    function setSelectOption() {
        ...
        if(shangjiaXingjiTypesSelect != null && shangjiaXingjiTypesOptions != null  && shangjiaXingjiTypesOptions.length > 0 ) {
            for (var i = 0; i < shangjiaXingjiTypesOptions.length; i++) {
                if (shangjiaXingjiTypesOptions[i].codeIndex == ruleForm.shangjiaXingjiTypes) {//下拉框value对比,如果一致就赋值汉字
                        shangjiaXingjiTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.shangjiaContent != null && ruleForm.shangjiaContent != 'null' && ruleForm.shangjiaContent != '' && $("#shangjiaContentupload").length>0) {

            var shangjiaContentUeditor = UE.getEditor('shangjiaContentEditor');
            shangjiaContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.shangjiaContent != null){
                    mes = ''+ ruleForm.shangjiaContent;
                    // mes = mes.replace(/\n/g, "<br>");
                }
                shangjiaContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#shangjiaContentupload").length>0) {
            var shangjiaContentEditor = UE.getEditor('shangjiaContentEditor');
            if (shangjiaContentEditor.hasContents()) {
                $('#shangjiaContent-input').attr('value', shangjiaContentEditor.getContent());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function shangjiaPhoneChickValue(e){
        ...
        function shangjiaDeleteChickValue(e){
            var this_val = e.value || 0;
            var reg=/^[1-9][0-9]*$/;
            if(!reg.test(this_val)){
                e.value = "";
    ...
    // 获取当前详情
    function getDetails() {
        var addshangjia = window.sessionStorage.getItem("addshangjia");
        if (addshangjia != null && addshangjia != "" && addshangjia != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("shangjia/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
